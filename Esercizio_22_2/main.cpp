 ...
 * @date    30/01/2024
 * @brief   Add to class fract_t of Esercizio_18_3 the code to generate an
 *          exception when a division by zero happens. Furthermore, add the
 *          code to generate an exception when a non valid number has been read.
 */
...
namespace fract_t
{
    class fract_err_t
    {
        public:
            ...
            fract_err_t (const std::string& error) : what(error)
            {
                /* Do nothing */
            }
    };
    ...
    class fract_t
    {
        public:
            ...
            fract_t (std::string frac)
            {
                ...
                {
                    throw fract_err_t(frac + " is not a fraction!");
                }
                ...
                {
                    case 1:
                        throw fract_err_t("Denominator cannot be 0");
                    break;
...
main ()
{
    ...
    catch (const fract_t::fract_err_t& err)
    {
        std::cerr << "Error encountered. Code: " << err.what << '\n';
        exit(8);
    }
    catch (...)
    {
